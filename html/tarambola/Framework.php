...
final class Dispatcher
{
    ...
    public static function getController()
    {
        ...
        // if it's a plugin and not activated, revert to tarambola hardcoded default
...
class Record
{
    ...
    //função para colocar a flag (description) dos formularios a read
    public function toReadForm($id)
    {
            $sql = 'UPDATE '.self::tableNameFromClassName(get_class($this)).' SET description="tara_read" WHERE id = '.$id;
            $return = self::$__CONN__->exec($sql) !== false;
            self::logQuery($sql);
            return($return);
    }
 ...
 *
 * In the tarambola class you can define what urls / routes map to what Controllers and
