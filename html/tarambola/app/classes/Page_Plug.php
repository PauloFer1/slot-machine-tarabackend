...
class Page_Plug
{
    ...
    public function children($args=null, $value=array(), $include_hidden=false)
    {
        ...
        $sql = 'SELECT page.*, author.name AS author, author.id AS author_id, updator.name AS updator, updator.id AS updator_id '
             ...
             . 'WHERE parent_id = '.$this->id.' AND (status_id='.Page_Plug::STATUS_REVIEWED.' OR status_id='.Page_Plug::STATUS_PUBLISHED.($include_hidden ? ' OR status_id='.Page_Plug::STATUS_HIDDEN: '').') '
    ...
    public static function find($uri) { return find_page_by_uri($uri); }
    ...
    {
        if ($this->needs_login == Page_Plug::LOGIN_INHERIT && $this->parent)
