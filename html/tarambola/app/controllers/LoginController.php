...
class LoginController extends Controller
{
    ...
    private function _sendPasswordTo($email)
    {
        ...
        if ($user)
        {
            ...
            $email = new Email();
            $email->from('no-reply@madebytarambola.com', 'tarambola CMS');
            $email->to($user->email);
            $email->subject('Your new password from tarambola CMS');
    ...
    /**
     * Checks what the latest tarambola version is that is available at madebytarambola.com
     *
    ...
    private function _checkVersion()
    {
        ...
        $v = file_get_contents('http://www.madebytarambola.com/version/', 0, $ctx);
        if ($v > tarambola_VERSION)
        {
            Flash::set('error', __('<b>Information!</b> New tarambola version available (v. <b>:version</b>)! Visit <a href="http://www.madebytarambola.com/">http://www.madebytarambola.com/</a> to upgrade your version!',
                       array(':version' => $v )));
        }
    }
