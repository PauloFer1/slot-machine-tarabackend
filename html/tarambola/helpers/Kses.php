 ...
 * @version 0.2.2
 * @since tarambola version beta 1
...
###############################################################################
{
 /*return preg_replace('%(<'.   # EITHER: <
  ...
  return preg_replace_callback('%(<'.   # EITHER: <
                      ...
                      '(>|$)'. # > or end of string
                      '|>)%', # OR: just a >
                      function($allowed_html, $allowed_protocols)
                        {return  kses_split2('\\1', $allowed_html, $allowed_protocols);},
                      $string);
} # function kses_split
...
# Change back the allowed entities in our entity whitelist
  ...
  $string = preg_replace_callback('/&amp;([A-Za-z][A-Za-z0-9]{0,19});/', function($matches){return '&\\1;';}, $string);
  
  //$string = preg_replace('/&amp;#0*([0-9]{1,5});/e', 'kses_normalize_entities2("\\1")', $string);
  $string = preg_replace_callback('/&amp;#0*([0-9]{1,5});/', function($matches){return 'kses_normalize_entities2("\\1")';}, $string);
  //$string = preg_replace('/&amp;#([Xx])0*(([0-9A-Fa-f]{2}){1,2});/', '&#\\1\\2;', $string);
  $string = preg_replace_callback('/&amp;#([Xx])0*(([0-9A-Fa-f]{2}){1,2});/', function($matches){return'&#\\1\\2;';}, $string);
