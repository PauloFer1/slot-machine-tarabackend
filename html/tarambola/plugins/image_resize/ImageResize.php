...
class ImageResize {
    ...
    public static function image_scale($source, $destination, $width, $height) {
        ...
        if ($width > $info['width'] && $height > $info['height']) {
           return self::image_gd_same($source, $destination, $width, $height);
        }
    ...
    public static function image_get_info($file) {
        ...
        if (!$data) {
            ...
            $extensions = array(1 => 'gif', 2 => 'jpeg', 3 => 'png',4 =>'jpg',  15 => 'wbmp');
    ...
    protected static function image_gd_resize($source, $destination, $width, $height, $source_x = 0, $source_y = 0, $source_width = null, $source_height = null) {
        ...
        if($info['format']=='png')
        {
            imagecolortransparent($res, imagecolorallocatealpha($res, 0, 0, 0, 127));
  ...
  protected static function image_gd_same($source, $destination, $width, $height, $source_x = 0, $source_y = 0, $source_width = null, $source_height = null)
  {
       if (!file_exists($source)) {
            return false;
        }
        ...
        $res = imagecreatetruecolor($source_width, $source_height);
        
        if($info['format']=='png')
        {
            imagecolortransparent($res, imagecolorallocatealpha($res, 255, 255, 255, 127));
            imagealphablending($res, false);
            imagesavealpha($res, true);
        }
        //$x = $width/2 - $source_width/2;
        //$y = $height/2 - $source_height/2;
        imagecopyresampled($res, $im, $x, $y, $source_x, $source_y, $source_width-1, $source_height-1,  $source_width, $source_height);
        if (!imageistruecolor($im)) {
            imagetruecolortopalette($res, false, 256);
        }
         $result = self::image_gd_write($res, $destination, $info['format']);
    ...
    protected static function image_gd_write($res, $destination, $format) {
        ...
        if (DEBUG) {
            // If tarambola is in debug mode, don't output to a file
            $destination = NULL;
