...
Plugin::setInfos(array(
    ...
    'website'     => 'http://www.naehrstoff.ch/code/image-resize-for-tarambola',
    'update_url'  => 'http://tarambola.naehrstoff.ch/plugin-versions.xml',
   ...
   /* if (!AuthUser::hasPermission('administrator,developer,editor')) {
    ...
    if (preg_match('#\.(jpe?g|gif|png|wbmp)$#i', CURRENT_URI)) {
        ...
        if (image_resize_scale(CURRENT_URI) && !DEBUG) {
            // If tarambola isn't debugging, it writes to a file; redirect to it
...
function image_resize_scale($path) {
    ...
    $server_path = tarambola_ROOT  . "/" . join("/", $params);
