...
switch($_REQUEST['t']){
		...
		switch(true){ default:
			...
			if($member_id){
				...
				if($newsletter->settings['subscribe_redirect']){
					...
					$message .= '<div style="background-color:#ffffff; opacity:0.5; width:170px; height:40px; text-align:center; padding-top:15px;"><h5>Subscrição efectuada!</h5></div>';
				}
			}
		}
	...
	case "view":
		...
		if(isset($_REQUEST['small']) && is_file(_NEWSLETTERS_DIR . 'newsletter-'.$id.'.html')){
			...
			include(_NEWSLETTERS_DIR . 'newsletter-'.$id.'.html');
                        ...
                        echo(_NEWSLETTERS_DIR);
			echo '<h3>Newsletter inexistente</h3>
	...
	case "unsub_form":
		...
		if($member_id){
			...
			$message = 'O seu e-mail foi removido da base de dados!';
		}
