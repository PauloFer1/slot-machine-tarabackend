...
class newsletter{
	...
	public function __construct(){
		...
		//$this->base_href = $_SERVER['HTTP_HOST'] . dirname($_SERVER['REQUEST_URI']);
                $this->base_href = URL_PUBLIC."tarambola/plugins/newsletter/views";
				...
				foreach($links[2] as $link_id => $l){
					...
					if(!preg_match('/^\{/',$l) && !preg_match('/^#/',$l) && !(preg_match('/^\w+:/',$l) && !preg_match('/^http/',$l))){
						...
						if($send_id){
							...
							$newlink = $this->base_href.'/ext.php?t=lnk&id='.$link_id.'&sid={SEND_ID}&mid={MEMBER_ID}&mhash={MEMBER_HASH}';
						}
			...
			if(is_array($links[2])){
				foreach($links[2] as $link_id => $l){
					...
					if(!preg_match('/^\{/',$l) && !preg_match('/^#/',$l) && !(preg_match('/^\w+:/',$l) && !preg_match('/^http/',$l))){
						...
						$l = ltrim($l,'/');
                                             //   echo("l-----> ".$l);
                                                if(preg_match('/tarambola\/plugins\/newsletter/', $l) && !preg_match("/http/", $l))
                                                {
                                                    $l2=preg_split('/tarambola\/plugins\/newsletter\/views\//', $l);
                                                    $l=$l2[1];
                                               //      echo("l2----> ".$l);
                                                }
						$newlink = ((!preg_match('/^http/',$l)) ? $base : '') . $l;
						...
						if(!isset($this->settings['inline_images']) || $this->settings['inline_images'] == 'no' || !$this->settings['inline_images']){
							if($send_id){
								...
								$res = query($sql,$db);
								$link_id = mysql_insert_id($db);
								$newlink =$this->base_href.'/ext.php?t=img&id='.$link_id.'&sid={SEND_ID}&mid={MEMBER_ID}';
							}
						}
						$replace = $type.'="'.$newlink.'"';
						//echo $replace."<br>\n";
	...
	public function write_newsletter_content($db,$newsletter_id){
		...
		$replace = array(
			...
			"unsubscribe_url" => $this->base_href.'/ext.php?t=unsub',
			"view_online"=>$this->base_href.'/ext.php?t=view&id='.$newsletter_id.'',
			"SENDTOFRIEND"=>$this->base_href.'/ext.php?t=send_to_friend&id='.$newsletter_id.'',
			"link_account"=>$this->base_href.'/ext.php?t=update_form',
		);
	...
	public function send_out_newsletter($db,$send_id,$member_id,$newsletter_id=false,$force=false){
		...
		if($status){
			...
			$replace = array(
				...
				"sent_month" => date("M Y"),
				/*"unsubscribe_url" => $this->base_href.'/ext.php?t=unsub&sid='.$send_id.'&mid='.$member_data['member_id'].'&hash='.md5("Unsub ".$member_data['member_id']."from $send_id").'',
				"view_online"=>$this->base_href.'/ext.php?t=view&id='.$send_data['newsletter_id'].'&sid='.$send_id.'&mid='.$member_data['member_id'].'&hash='.md5("view link ".$member_data['member_id']."from $send_id").'',
	...
	public function get_form($db,$include_form_tag = false,$member_id=false,$admin=false){
		...
		<form action="<?php echo $this->base_href;?>/ext.php?t=signup" method="post">
		<?php } ?>
