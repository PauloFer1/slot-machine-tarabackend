...
if(isset($_REQUEST['save'])){
	...
	if(isset($_REQUEST['next'])){
		ob_end_clean();
		header("Location: ?p=open&newsletter_id=$newsletter_id");
		exit;
	}
	...
	switch ($_REQUEST['next_action_key']){
		case 'delete_content':
			if(!$errors){
				...
				ob_end_clean();
				header("Location: plugin/newsletter/index.php?p=create&newsletter_id=$newsletter_id");
				exit;
			}
			break;
		case 'swap_content':
			if(!$errors){
				ob_end_clean();
				header("Location: plugin/newsletter?p=create&newsletter_id=$newsletter_id&newsletter_content_id=".$_REQUEST['next_action_val']);
				exit;
			}
			break;
		case 'preview':
			if(!$errors){
				ob_end_clean();
				header("Location: ?p=preview&newsletter_id=$newsletter_id&hide_menu=true");
				exit;
			}
			break;
		case 'preview_email':
			if(!$errors){
				ob_end_clean();
				header("Location: ?p=preview&newsletter_id=$newsletter_id&hide_menu=true&email=".urlencode($_REQUEST['next_action_val']));
				exit;
			}
			break;
	}
	...
	setup : function(ed){
		ed.onNodeChange.add(function(ed, cm, n, co) {
			if(n.nodeName == 'IMG'){
				...
				jQuery('#image_size').val('replace');
				jQuery('#image_alt').val(ed.dom.getAttrib(n, 'alt'));
				image_width = jQuery(n).width();
				image_height = jQuery(n).height();
			}else{
				jQuery('#image_size').val('');
...
<h1 class="news"><? echo __('Create Newsletter')?></h1>

<form action="plugin/newsletter?p=create&save=true&next=true" method="post" id="create_form" enctype="multipart/form-data">
...
<h2 class="recent"><span><? echo __('Step 1');?>:</span> <? echo __('Template');?></h2>
...
<div class="box templates">
	...
	<div class="template<?php echo ($newsletter_data['template']==$template['name'])?' selected':'';?>" rel="<?php echo $template['name'];?>">
	        <img src="<?php echo (URL_PUBLIC.'tarambola/plugins/newsletter/views/templates/'.$template['name']);?>/preview.jpg" border="0">
		<div style="clear:both; padding:5px;"><?php echo $template['name'];?></div>
	</div>
...
jQuery('.template').click(function(){
	jQuery('.templates .selected').removeClass('selected');
	jQuery(this).addClass('selected');
	jQuery('#template').val(jQuery(this).attr('rel'));
	// prompt to re-load with content available
	<?php if($input_method != 'wizard'){ ?>
	var reload_content = confirm('Pretende usar este template?');
	if(reload_content){
		jQuery('#template_reload').val('1');
	}
	<?php } ?>
	jQuery('#create_form')[0].action='plugin/newsletter?p=create&save=true#editor';
	jQuery('#create_form')[0].target='_self';
	jQuery('#create_form')[0].submit();
	return false;
});
</script>


<h2 class="recent"><span><? echo __('Step 2');?>:</span> <? echo __('Settings');?></h2>

<div class="box">
	<table cellpadding="5">
		<tr>
			<td>
				<? echo __('Subject of Email');?>: <span class="required">*</span>
			</td>
			<td>
		...
		<tr>
			<td>
				<? echo __('From (Name)');?>: <span class="required">*</span>
			</td>
			<td>
		...
		<tr>
			<td>
				<? echo __('From (Email)');?>: <span class="required">*</span>
			</td>
			<td>
		...
		<tr>
			<td>
				<? echo __('Return Email');?>: <span class="required">*</span>
			</td>
			<td>
				<input type="text" class="input" name="bounce_email" value="<?php echo $newsletter_data['bounce_email'];?>"> (<? echo __('Newsletter returned to this address');?>)
			</td>
		</tr>
		<tr>
			<td>
...
<h2 class="recent"><span><? echo __('Step 3');?>:</span> <? echo __('Content');?></h2>
			...
			<td valign="top">
				<table cellpadding="5">
					<tr>
						...
						<td valign="top">
							<div id="image_insert">
								<h3><? echo __('Insert Image')?>:</h3>
								<?php if($input_method=='wizard'){ ?>
								<? echo __('The main image will appear above the top of the article');?>.<br>
								<?php } ?>
								<select name="image_url" id="image_url">
								<option value=""> #1: <? echo __('Select Image');?> </option>
								<?php
								...
								<br>
								<select name="image_size" id="image_size" onchange="jQuery('#image_alt')[0].focus().select();">
								<option value=""> #2: <? echo __('select Size');?> </option>
                                                                <option value="replace"><? echo __('Replace existing');?></option>
								<option value="200x200">Thumbnail #1 - 100x100</option>
                                                                <option value="100x200">Thumbnail #2 - 100x200</option>
                                                                <option value="200x100">Thumbnail #3 - 200x100</option>
                                                                <option value="180x320">Thumbnail #4 - 180x320</option>
                                                                <option value="320x180">Thumbnail #5 - 320x180</option>
								</select>
								<br>
								<input type="text" name="image_alt" id="image_alt" value="Image Description" onfocus="if(this.value=='<? echo __('Image Description');?>')this.value='';">
								<br>
								<input type="button" name="image_insert" onclick="insert_image();" value="<? echo __('Insert Image');?>">
								<a href="#" onclick="jQuery('#image_insert').hide(); jQuery('#image_upload').show(); return false;"><? echo __('Upload new image');?></a>
							</div>
							<div id="image_upload" style="display:none;">
								<h3><? echo __('Upload new image');?>:</h3>
								<input type="file" name="image" value="" size="6"> <br>
								<input type="submit" name="attach" value="Upload" onclick="this.form.action='plugin/newsletter?p=create&save=true#editor'; this.form.target='_self';">
								<a href="#" onclick="jQuery('#image_upload').hide(); jQuery('#image_insert').show(); return false;"><? echo __('Insert Existing Image');?></a>
							</div>
							<script language="javascript">
							function insert_image(){
								var src = jQuery('#image_url').val();
								jQuery('#image_url').val('');
								var size = jQuery('#image_size').val();
								jQuery('#image_size').val('');
								...
								var imgUrlArr = src.split("<?php echo(SERVER_URL); ?>");
                                                                var imgUrl = "<?php echo(URL_PUBLIC); ?>"+imgUrlArr[1];
								var imghtml = '<img src="' + imgUrl + '" alt="'+alt+'"';
								if(size == 'replace'){
								}else if(size != ''){
									// split and use size.
...
<!--<h2><span>4º Passo:</span> Preview</h2>

<div class="box">
	<table cellpadding="5">
		<tr>
			<td>
				Pre-visualizar no Browser
			</td>
			<td>
				<input type="submit" name="preview1" value="Open Preview" onclick="jQuery('#next_action_key').val('preview');">
			</td>
		</tr> 
		<tr>
			<td>
				Pre-visualizar no Email
			</td>
			<td>
				 <input type="text" name="preview_email" id="preview_email" value=""><input type="submit" name="preview2" value="Enviar" onclick="jQuery('#next_action_key').val('preview_email');jQuery('#next_action_val').val(jQuery('#preview_email').val());">
			</td>
		</tr>
	</table>
...
<h2 class="recent"><span><? echo __('Step 4')?>:</span> <? echo __('Save');?></h2>

<div class="box">
	<p><? echo __('Save newsletter to go to the next step.');?></p>
        <input type="submit" name="save_cont" value="<? echo __('Save and Continue');?>..." onclick="this.form.action=<?php echo(URL_PUBLIC); ?>'tarabackend/plugin/newsletter?p=create&save=true&next=true'; this.form.target='_self';">
</div>
