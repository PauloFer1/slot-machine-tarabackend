...
if(!$newsletter_id){
	...
	echo 'Por favor volte atrás e selecione uma newsletter';
}
...
if(isset($_REQUEST['delete'])){
	...
	ob_end_clean();
	header("Location: plugin/newsletter?p=past");
	exit;
}
		...
		if($newsletter_id){
			if($_REQUEST['send']){
				...
				if(is_array($_REQUEST['group_id'])){
					...
					$errors [] = "Seleccione um grupo para o envio!";
				}
				...
				if(!$errors){
					...
					if(!$send_id){
						$errors[] = "Nenhum membro encontrado";
					}else{
						ob_end_clean();
						header("Location: ".URL_PUBLIC."tarabackend/plugin/newsletter?p=send&send_id=$send_id");
						exit;
					}
				}
			}else{
				ob_end_clean();
				header("Location: plugin/newsletter?p=open&newsletter_id=$newsletter_id");
				exit;
			}
		}else{
			$errors [] = 'Erro ao criar newsletter na Base de Dados!';
		}
	}
...
<form action="<?php echo(URL_PUBLIC); ?>tarabackend/plugin/newsletter?p=open&save=true" method="post" id="create_form">

<input type="hidden" name="newsletter_id" value="<?php echo $newsletter_id;?>">

<a href="#" onclick="jQuery('#other_settings').slideToggle(); return false;">show settings / edit newsletter again</a>
...
<div class="box">
	<table cellpadding="5">
		...
		<tr>
			<td>
				Não enviar a subscritores que já receberam esta newsletter:
			</td>
			<td>
		...
		<tr>
			...
			<td>
				 <input type="submit" name="send" value="Enviar<?php echo (count($sends))?' novamente':'';?>!">
			</td>
		</tr>
	</table>
</div>
			...
			<table cellpadding="5">
				...
				foreach($pending as $send){
					...
					<tr>
						...
						<td><a href="plugin/newsletter?p=send&send_id=<?php echo $send['send_id'];?>">Continuas a enviar</a></td>
					</tr>
					<?php
				}
				?>
...
<div class="box">
	<table cellpadding="5">
		...
		<tr>
			...
			<td>
				<a href="plugin/newsletter?p=stats&newsletter_id=<?php echo $newsletter_id;?>&send_id=<?php echo $send['send_id'];?>">View Stats</a>
			</td>
		</tr>
		<?php } ?>
