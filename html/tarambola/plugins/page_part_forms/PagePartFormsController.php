            ...
            switch ($element[self::PROPERTY_TYPE]) {
                ...
                case self::TYPE_TABLE:
                 ...
                 case self::TYPE_ATTRIBUTES:
                    // Limit selection
                    self::Array_get_set($structure_element, $element, self::PROPERTY_LIMIT, 1000);
                    break;
                default:
            ...
            if ($part == "add") {
                ...
                foreach ($mods as $m) {
                    foreach ($m as $key => $value) {
                        if ($inc > 1) {
                            array_push($parents, Page::findById(($parents[$inc - 2]->parent_id)));
                        }

                        if (($pageId == $key) || ($inc > 1 && $parents[$inc - 2]->parent_id == $key)) {
                            if ($form = PageMetadata::FindOneByPageAndKeyword($key + $inc, self::PLUGIN_ID)) {
