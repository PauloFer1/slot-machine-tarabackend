...
class FileUpload {
  ...
  public function handleUpload($uploadDir = null, $allowedExtensions = null) {
    ...
    $i=1;
    while(file_exists($this->uploadDir.$this->fileName))
    {
        $temp3 = preg_split("/\.([^\.]+)$/", $this->fileName);
        preg_match("/\.([^\.]+)$/", $this->fileName, $temp2);
        $targetFile =  $this->uploadDir.$temp3[0].'-'.$i.$temp2[0];
        $this->fileName = $temp3[0].'-'.$i.$temp2[0];
        $i++;
    }
