...
function url_start_with($url)
{
              ...
              case PagePartFormsController::TYPE_ATTRIBUTES:
                  ...
                  echo('<div id="allAttributes">');
                        ...
                        foreach ($atributos as $att):    
                                ...
                                $attrBoxHid.='</select>';
                        endforeach;
                        $attrBoxHid.='<input class="quantidadeAtt" type="number" value=""/>
  ...
  //--------------------------imagem-------------------------------------------------------------
    ...
    <script>
    initUploader("'.URL_PUBLIC.'", "getReadyBtnImg_'.str_replace("-","_",$structure_name_css_id).'", "uploadFileBlockImg_'.str_replace("-","_",$structure_name_css_id).'","'.$structure_name_css_id.'");
...
<a href="#" id="getReadyBtnImg_'.str_replace("-","_",$structure_name_css_id).'" class="uploadImgBtn">Carregar Imagens</a>
...
jQuery(\'#file_upload_file_'.str_replace("-","_",$structure_name_css_id).'\').uploadify({
	...
	\'onAllComplete\' : function(event,data) {
      alert(data.filesUploaded + \' ficheiro(s) submetidos com sucesso! Por Favor guarde a página para associar.\');
    }
  });
