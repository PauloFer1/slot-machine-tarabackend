...
v2.0.0 - Removed TinyMCE's tab in tarambola backend
         Moved plugin's settings DB table stuff into enable.php
         Added a settings page

v1.2.0 - Added a configuration/preferences tab to tarambola's admin section.
         Added an option to also list HIDDEN pages that are unprotected in TinyMCE's link dropdown box.

v1.1.0 - Added dropdown in-tarambola pages selector for easy linking
...
1 - Unzip and upload this plugin to the tarambola plugins directory.

2 - IF NECESSARY, edit the tinymce.js file and change the "urlTotarambola" variable.
    This variable should be set to the server relative url in which tarambola was installed.
    For example:
        - if you access tarambola's frontpage with http://www.example.com/tarambola
        - The value for urlTotarambola should be "/tarambola"
        - When you tarambola lives on your site's root, leave this value empty.

3 - Access the tarambola Administration screen and activate the plugin. Refresh/reload the screen and
